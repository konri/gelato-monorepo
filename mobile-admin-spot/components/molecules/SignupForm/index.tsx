...
import { persistPendingVerificationEmail } from "@/utils/emailVerificationFlow";
...
export const SignupForm = ({ }: SignupFormProps) => {
  ...
  const handleSubmit = async (data: SignupFormData) => {
    ...
    if (response.error) {
      if (response.errorCode === AUTH_ERROR_CODE.EMAIL_EXISTS_UNVERIFIED) {
        await persistPendingVerificationEmail(data.email, {
          allowImmediateResend: true,
        });
        return;
      }
      if (
        response.errorCode === AUTH_ERROR_CODE.EMAIL_EXISTS_VERIFIED ||

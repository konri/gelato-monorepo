...
export const getRandomRotation = (): number => {
  return (Math.random() - 0.5) * ROTATION_RANGE;
};

export const getRandomOffset = (): StampOffset => {
  return {
    x: (Math.random() - 0.5) * OFFSET_RANGE,
    y: (Math.random() - 0.5) * OFFSET_RANGE,
  };
};
...
export const calculateSymmetricalLayout = (
  totalStamps: number
): number[] => {
  if (totalStamps <= 4) {
    return [totalStamps];
...
export const calculateStampGrid = (
  totalStamps: number,
  stampsPerRow: number
) => {
